...
def login(request):
   ...
   if request.user.is_authenticated():
      ...
      if len(request.user.last_name)==0:
         if 'sn' in  request.META: request.user.last_name = request.META['sn']
         else: request.user.last_name = request.user.username
         if 'givenName' in  request.META: request.user.first_name = request.META['givenName']
         else: request.user.first_name = 'Joe'
         logger.info('New User name=' + request.user.first_name + ' ' + request.user.last_name)
      ...
      if 'isMemberOf' in request.META:
         ...
         grps = grpstr.split(';')
         for g in grps:
            if re.search('^urn:mace:washington.edu:groups:u_netid_.*_admins$', g) != None:
               a = re.sub(r'urn:mace:washington.edu:groups:u_netid_(.*)_admins',r'\1',g)
               aliases.append(a)
...
def index(request, template=None):
      ...
      remote_user = re.sub(r'@.*','',request.user.username)  # remove the @washington.edu
